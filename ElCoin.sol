...
contract ElCoin is Context, IERC20, IERC20Metadata,Ownable {
    ...
    function setRestriction(bool status) public onlyOwner{
        restriction=status;
    }
    ...
    function _transfer(
        ...
        if (pairAddress==address(0)){
            require(sender==owner()||recipient==owner(),"Trading hasn't yet started");
        }
        ...
        if(sender==pairAddress && takeFee){
            uint256 taxFee = (amount.mul(buyTax)).div(100);
        ...
        if(recipient==pairAddress && takeFee){
            uint256 taxFee = (amount.mul(sellTax)).div(100);
            _balances[taxAddress] = _balances[taxAddress].add(taxFee);
            emit Transfer(sender, taxAddress, taxFee);
            amount = amount.sub(taxFee);
        }

        if (restriction && takeFee){
            require((_balances[recipient].add(amount))<((_totalSupply.mul(5)).div(100)),"Forbidden");
        }
